#
# This scripts looks up definitions on urbandictionary.com.
#
...
# url: the url to fetch. if this is blank it will be generated based on the
#   query and number. this might be provided if we are following an http
#   redirect.
...
proc ::ud::fetch {server channel url query number request_count} {
	...
	# the requested definition may be on a different page.
	lassign [::ud::find_term_by_page $number] page

	# TODO: if we have a url from a redirect it appears it does not include
	#   the page parameter we may have requested... though maybe this is only
	#   the case if there is only a single page of actual results? regardless
	#   this can mean our number parameter is not really correct if we follow
	#   a redirect.
	#   for now I am choosing to append the page in that case... this will
	#   be a problem if the location header changes to include the page of course!
	if {[string length $url] > 0} {
		set url ${url}&page=$page
	# we may already have the url to request to.
	} else {
